...
/* Tambahkan gaya CSS di file stylesheet Anda */
...
<body>
    <div id="nav">
        ...
        <ul>
            <li><a href="<?= base_url('user-home'); ?>"><i class="ri-apple-fill"></i> Store</a></li>
       ...
       <!-- application/views/user_notifikasi.php -->

<div id="userIcon" class="notification-icon">
    <a href="#">
        <i class="ri-bell-line"></i>
        <span class="badge"><?= count($notifications); ?></span>
    </a>
    <!-- Dropdown content -->
    <div id="" class="notification-dropdown">
        <?php foreach ($notifications as $notification) : ?>
            <div class="notification-item">
                <div class="notification-title"><?= $notification->title; ?></div>
                <div class="notification-message"><?= $notification->message; ?></div>
                <div class="notification-time"><?= $notification->jam_msg; ?></div>
            </div>
        <?php endforeach; ?>
    </div>
</div>
      ...
      <div id="userIcon">
        ...
        <div id="userIcon">
            ...
            <?php
                ...
                $user_photo = $user_data->foto; // Ambil nama file foto pengguna dari sesi
                if ($user_photo) {
                    $photo_path = base_url('upload/' . $user_photo);
                    echo '<img src="' . $photo_path . '" alt="User Photo" style="border-radius: 50%; width: 24px; height: 24px;">';
                } else {
                    echo '<i class="ri-user-fill"></i>';
                }
            ?>
            </a>
            <!-- Dropdown content -->
            <div id="userDropdown">
            <a href="<?= base_url('user/data_user'); ?>">
                    <i class="ri-user-fill"></i> Akun Saya
                </a>
                <a href="<?= base_url('user/pesanan_full'); ?>">
                    <i class="ri-file-text-fill"></i> Pesanan
                </a>
                <a href="<?= base_url('user/pesanan_full'); ?>">
                    <i class="ri-history-fill"></i> Riwayat Pesanan
                </a>
                <a href="<?php echo site_url('login/logout')?>">
                    <i class="ri-login-box-fill"></i> Log Out
                </a>
            </div>
        </div>
    </div>
...
function openModal(modalId) {
    ...
    $(document).ready(function() {
        function getRealtimeNotifications() {
            $.ajax({
                url: '<?= base_url('user/get_realtime_notifications'); ?>',
                type: 'GET',
                dataType: 'json',
                ...
                complete: function() {
                    ...
                    setTimeout(getRealtimeNotifications, 5000); // Set interval in milliseconds (5 seconds in this example)
                }
            });
        }

        function updateNotifications(notifications) {
            // Update your UI with the new notifications data
            // Example: $("#userDropdown").html(...);
